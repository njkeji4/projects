...
THREEx.ProceduralCity	= function(renderer){
	...
	this.createSquareBuildings	= function(){
		...
		for( var blockZ = 0; blockZ < nBlockZ; blockZ++){
			for( var blockX = 0; blockX < nBlockX; blockX++){
					...
					if((blockZ == 4 || blockZ == 5 ) && (blockX == 4 || blockX == 5 )){
	...
	this.ellipseBuilding = function(){
		...
		{
			geometry.faceVertexUvs[0][i][0].set(0,1);
			geometry.faceVertexUvs[0][i][1].set(0,0);
			geometry.faceVertexUvs[0][i][2].set(1,1);
			geometry.faceVertexUvs[0][i+1][0].set(0,0);
			geometry.faceVertexUvs[0][i+1][1].set(1,0);
			geometry.faceVertexUvs[0][i+1][2].set(1,1);
		}
		//console.log(new THREE.BoxGeometry(1,1,1));
		var textureLoader = new THREE.TextureLoader();
		var texture	= textureLoader.load( "./texture/glassbuilding.png");//glassbuilding.png" );
	...
	this.createSquareCity	= function(){
		...
		var ellipseMesh = this.ellipseBuilding();
		object3d.add(ellipseMesh)	
	...
	this.colorifyBuilding	= function(buildingMesh){
		...
		for ( var j = 0, jl = geometry.faces.length; j < jl; j ++ ) {
			if ( j === 4 || j=== 5 ) {
				...
				geometry.faces[ j ].color=baseColor;//.vertexColors = [ baseColor, baseColor, baseColor, baseColor ];
			} else {
				// set face.vertexColors on sides faces
				geometry.faces[ j ].color=bottomColor;//.vertexColors = [ topColor, bottomColor, bottomColor, topColor ];
			}
		}		
	}
